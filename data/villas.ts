 ...
 *
 * ---------------------------------------------------------------------------
 * BURAYI DÜZENLEYENLER İÇİN: her proje aşağıdaki `villaProjects` dizisinde
 * `slug` alanına göre bulunur (örn. `slug: "dagyoncali"`). Sık düzenlenecek
 * alanlar:
 *   - title        → proje başlığı (tr/en)
 *   - excerpt      → kart üzerindeki kısa özet (tr/en)
 *   - description  → proje detay sayfasındaki paragraflar (tr/en, her biri
 *                    ayrı bir paragraf olacak şekilde dizi elemanı)
 *   - highlights   → detay sayfasında rozet olarak çıkan kısa maddeler (tr/en)
 *   - priceRangeTRY, parcelCount, parcelAreaRange, adaParsel, projectAreaM2
 *                  → resmi proje rakamları
 * FOTOĞRAFLAR koda değil, `public/images/projeler/<slug>/` klasörüne eklenir
 * — aşağıdaki `projectImages()` fonksiyonuna ve kök dizindeki
 * `ICERIK-REHBERI.md` dosyasına bakın.
 * ---------------------------------------------------------------------------
 *
